...
def build_invdyn_controller():
    ...
    controller.weight_contact_forces.value = [1.0, 1.0, 1e-3]

    # Gains.
    def set_gain(name, value, dim):
        controller.__getattr__("kp_" + name).value = dim * [value]
        controller.__getattr__("kd_" + name).value = dim * [
            2.0 * np.sqrt(value)
        ]

    set_gain("com", 1.0, 3)
    set_gain("feet", 1.0, 6)
    set_gain("constraints", 100.0, 6)

    # Signals for the contacts.
    for cf in contact_frames:
        controller.__getattr__(cf + "__f_min").value = 0.0
        controller.__getattr__(cf + "__f_max").value = 10.0
        controller.__getattr__(cf + "__contact_normal").value = [0.0, 0.0, 1]
    ...
    controller.com_ref_vel.value = 3 * [0.0]
    controller.com_ref_acc.value = 3 * [0.0]
