...
def get_q_ref_from_4silders(sliders, max_hip_joint, entity_name):
    ...
    for i, leg in enumerate(["front", "hind"]):
        ...
        plug(
            q_ref.__dict__[leg + "_hip_qref"].sout,
        ...
        plug(
            q_ref.__dict__[leg + "_knee_qref"].sout,
...
def get_q_ref_from_interpolation(q_signal, max_hip_joint, entity_name):
    ...
    q_ref.go_0 = cubic_interpolator(q_signal, q_ref.q_0, name + "_go_0")
    #  _
    # < <
...
def get_joint_controller(robot):
    ...
    jt_ctrl.Kp = constVector(
        8 * [jt_ctrl.solo_config.kp], jt_ctrl.name + "_Kp"
    )
    jt_ctrl.Kd = constVector(
        8 * [jt_ctrl.solo_config.kd], jt_ctrl.name + "_Kd"
    )
