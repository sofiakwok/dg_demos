...
from dg_demos.solo12.controllers.centroidal_controller import (
    get_controller as ctrl_get_controller,
)
...
class Solo12PlanWbc(object):
    def __init__(self, prefix):
        ...
        self.config_path = os.path.join(
            ...
            "python/dg_demos/solo12/plans/cfg_solo12_submission_parkour/net/",
        )

        self.ctrl = ctrl_get_controller(prefix + "_wbc")

        self.kp_joint, self.kp_joint_entity = constDouble(
            0.0, prefix + "_kp_joint", True
        )
        self.kd_joint, self.kd_joint_entity = constDouble(
            0.0, prefix + "_kd_joint", True
        )
        ...
        self.target_pos_sin, self.target_pos = constVectorOp(
            12
            * [
                0.0,
            ],
        ...
        self.target_dur_sin, self.target_dur = constVectorOp(
            4
            * [
                0.0,
            ],
    ...
    def switchReader(
        self, sin, default_value, config_filename, sel_value=None
    ):
    ...
    def plug(self):
        ...
        tau = DoubleSignal(self.kp_joint) * (
            VectorSignal(self.des_pos_joint, 12)
            - VectorSignal(robot.device.joint_positions, 12)
        ) + DoubleSignal(self.kd_joint) * (
            VectorSignal(self.des_vel_joint, 12)
            - VectorSignal(robot.device.joint_velocities, 12)
        )

        # Use PD controller with the centroidal controller.
        tau_res = VectorSignal(tau, 12) + VectorSignal(
            ctrl.control_torques, 12
        )
        plug(tau_res, self.robot.device.ctrl_joint_torques)

        self.switchReader(
            self.des_pos_joint_sin,
            self.default_joint_pos,
            "quadruped_generalized_positions.dat",
            "".join(12 * ["1"] + 8 * ["0"]),
        )
        self.switchReader(
            self.des_vel_joint_sin,
            self.default_joint_vel,
            "quadruped_generalized_velocities.dat",
            "".join(12 * ["1"] + 7 * ["0"]),
        )

        self.switchReader(
            ctrl.des_pos_com_sin, [0.0, 0.0, 0.2], "quadruped_com.dat"
        )
        self.switchReader(
            ctrl.des_vel_com_sin, [0.0, 0.0, 0.00], "quadruped_com_vel.dat"
        )
        self.switchReader(
            ctrl.des_fff_com_sin,
            [0.0, 0.0, Solo12Config.mass * 9.81],
            "quadruped_centroidal_forces.dat",
        )

        self.switchReader(
            ctrl.des_ori_com_sin,
            [0.0, 0.0, 0.0, 1.0],
            "quadruped_quaternion.dat",
        )
        self.switchReader(
            ctrl.des_ang_vel_com_sin,
            [0.0, 0.0, 0.0],
            "quadruped_base_ang_velocities.dat",
        )
        self.switchReader(
            ctrl.des_fft_com_sin,
            [0.0, 0.0, 0.0],
            "quadruped_centroidal_moments.dat",
        )

        self.switchReader(
            ctrl.contact_plan_sin,
            [1, 1, 1, 1],
            "quadruped_contact_activation.dat",
        )
        self.switchReader(
            ctrl.des_pos_eff_sin,
            [
                0.195,
                0.147,
                0.015,
            ...
            "quadruped_positions_abs.dat",
        )
        self.switchReader(
            ctrl.des_vel_eff_sin,
            [
                0.0,
                0.0,
                0.0,
...
if ("robot" in globals()) or ("robot" in locals()):
    ...
    # ground of the robot.
    wtrl.quad_com_ctrl.vicon_offset.value = [0.0, 0.0, 0.005]
